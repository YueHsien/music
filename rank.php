...
?>
	...
	<div class="bg2">
		...
		if (isset ( $_SESSION ["SQL"] ))
			...
			$sql = "SELECT total.音樂編號,title,name,region,author,click FROM member,total,uploader WHERE uploader.memberid=member.memberid AND uploader.音樂編號=total.音樂編號  ORDER BY click DESC";
		if (isset ( $_GET ["Pages"] ))
			$pages = $_GET ["Pages"];
		else
			$pages = 1;
	...
	<?php
	if ($pages > 1) // 顯示上一頁
		echo "<a href='rank.php?Pages=" . ($pages - 1) . "'>上一頁</a>| ";
	for($i = 1; $i <= $total_pages; $i ++)
		if ($i != $pages)
			echo "<a href=\"rank.php?Pages=" . $i . "\">" . $i . "</a> ";
		else
			echo $i . " ";
	if ($pages < $total_pages) // 顯示下一頁
		echo "|<a href='rank.php?Pages=" . ($pages + 1) . "'>下一頁</a> ";
