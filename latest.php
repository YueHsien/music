	...
	<div class="top_topmenu">
		<div class="top_topmenu_frame">
			<div class="float_left top_topmenu_left">
				...
				<div class="float_left newindex_menu_line"></div>
				<div class="newindex_menuitem float_left">
					<a href="first.php"><font size="5.5" color="#007799">首頁</font></a>
				</div>
			</div>
		</div>
	</div>
...
<?php
		...
		if (isset ( $_SESSION ["SQL"] ))
			...
			$sql = "SELECT total.音樂編號,title,name,region,author,click,上傳日期 FROM member,total,uploader WHERE uploader.memberid=member.memberid AND uploader.音樂編號=total.音樂編號  ORDER BY 音樂編號  DESC";
		if (isset ( $_GET ["Pages"] ))
			$pages = $_GET ["Pages"];
		else
			$pages = 1;
	...
	<?php
	if ($pages > 1) // 顯示上一頁
		echo "<a href='latest.php?Pages=" . ($pages - 1) . "'>上一頁</a>| ";
	for($i = 1; $i <= $total_pages; $i ++)
		if ($i != $pages)
			echo "<a href=\"latest.php?Pages=" . $i . "\">" . $i . "</a> ";
		else
			echo $i . " ";
	if ($pages < $total_pages) // 顯示下一頁
		echo "|<a href='latest.php?Pages=" . ($pages + 1) . "'>下一頁</a> ";
